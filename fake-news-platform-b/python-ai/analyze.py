...
def load_detection_rules():
    ...
    if not HAS_MYSQL:
        logger.warning("MySQL not available, using default rules")
        _rules_cache = get_default_rules()
...
def rule_based_classify(text: str, use_db_rules: bool = True) -> dict:
    ...
    scores = {"fake_news": 0.0, "hate_speech": 0.0, "misinformation": 0.0, "cyberbullying": 0.0, "reliable": 0.0}
    detected_keywords = []
    ...
    for category in ['fake_news', 'hate_speech', 'misinformation', 'cyberbullying']:
        if category not in rules:
            continue
    ...
    max_score = max(
        scores["fake_news"], scores["hate_speech"], scores["misinformation"], scores["cyberbullying"],
    )
    if max_score < 0.30:
        ...
        category = max(
            (k for k in ['fake_news', 'hate_speech', 'misinformation', 'cyberbullying']),
    ...
    # Niveau de risque
    if confidence >= 90 or category == "hate_speech" or category == "cyberbullying":
        risk = "critical"
    elif confidence >= 75:
        risk = "high"
    elif confidence >= 60:
...
def gbert_classify(text: str, rule_result: dict) -> dict:
    ...
    priority_rules = (
        "hate_speech", "cyberbullying", "misinformation",
    )
    if rule_cat in priority_rules and rule_conf >= 55:
        category = rule_cat
        conf = max(conf, rule_conf)
    elif pred == 1 and rule_cat == "misinformation" and rule_conf >= 50:
        category = "misinformation"
        conf = max(conf, rule_conf)

    if conf >= 90 or category in ("hate_speech", "cyberbullying"):
        risk = "critical"
    elif conf >= 75:
        risk = "high"
    elif conf >= 60:
...
def ai_classify(text: str) -> dict:
    ...
    try:
        ...
        # Risk level
        if final_conf >= 90 or final_cat == "hate_speech" or final_cat == "cyberbullying":
            risk = "critical"
        elif final_conf >= 75:
            risk = "high"
        elif final_conf >= 60:
...
def save_analysis(post_id: int, result: dict) -> int:
    ...
    if not HAS_MYSQL:
        logger.warning("MySQL not available, cannot save analysis")
        return 0
